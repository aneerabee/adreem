-- Add EUR as a fully posted ADREEM currency without rewriting existing rows.

alter table public.adreem_accounts
  add column if not exists balance_eur numeric(15, 0) not null default 0;

alter table public.adreem_accounts
  drop constraint if exists adreem_accounts_currency_kind_check;
alter table public.adreem_accounts
  add constraint adreem_accounts_currency_kind_check check (currency_kind in ('LYD', 'USD', 'TRY', 'EUR', 'multi'));
alter table public.adreem_accounts
  drop constraint if exists adreem_accounts_nonnegative_eur_check;
alter table public.adreem_accounts
  add constraint adreem_accounts_nonnegative_eur_check check (
    value_kind not in ('cash', 'bank', 'asset') or balance_eur >= 0
  );

alter table public.adreem_movements
  drop constraint if exists adreem_movements_currency_check;
alter table public.adreem_movements
  add constraint adreem_movements_currency_check check (currency is null or currency in ('LYD', 'USD', 'TRY', 'EUR'));

alter table public.adreem_movement_entries
  drop constraint if exists adreem_movement_entries_currency_check;
alter table public.adreem_movement_entries
  add constraint adreem_movement_entries_currency_check check (currency in ('LYD', 'USD', 'TRY', 'EUR'));
...
create or replace function adreem_private.sync_eur_entry_balance()
returns trigger
language plpgsql
security invoker
set search_path = ''
as $$
begin
  if tg_op = 'DELETE' and old.currency = 'EUR' then
    update public.adreem_accounts
    set balance_eur = balance_eur - old.delta
  ...
  if tg_op = 'INSERT' and new.currency = 'EUR' then
    begin
      update public.adreem_accounts
      set balance_eur = balance_eur + new.delta
...
revoke all on function adreem_private.sync_eur_entry_balance() from public, anon, authenticated, service_role;
drop trigger if exists adreem_sync_eur_entry_balance on public.adreem_movement_entries;
create trigger adreem_sync_eur_entry_balance
after insert or delete on public.adreem_movement_entries
for each row execute function adreem_private.sync_eur_entry_balance();

create or replace function adreem_private.sync_eur_entry_balance_update()
returns trigger
language plpgsql
security invoker
set search_path = ''
...
begin
  begin
    ...
    set balance_eur = account.balance_eur + net.delta
    from net
...
revoke all on function adreem_private.sync_eur_entry_balance_update() from public, anon, authenticated, service_role;
drop trigger if exists adreem_sync_eur_entry_balance_update on public.adreem_movement_entries;
create trigger adreem_sync_eur_entry_balance_update
after update on public.adreem_movement_entries
referencing old table as old_eur_entries new table as new_eur_entries
for each statement execute function adreem_private.sync_eur_entry_balance_update();

create or replace function adreem_private.protect_account_eur_lifecycle()
returns trigger
language plpgsql
security invoker
set search_path = ''
as $$
begin
  if tg_op = 'DELETE' and old.balance_eur <> 0 then
    raise exception 'ADREEM_ACCOUNT_DELETE_IN_USE' using errcode = '23514';
  end if;
  if tg_op = 'UPDATE' and old.status = 'active' and new.status = 'inactive' and new.balance_eur <> 0 then
...
drop trigger if exists adreem_protect_account_eur_lifecycle on public.adreem_accounts;
create trigger adreem_protect_account_eur_lifecycle
before update or delete on public.adreem_accounts
for each row execute function adreem_private.protect_account_eur_lifecycle();
...
begin
  ...
  v_suffix := case new.currency when 'USD' then 'usd' when 'TRY' then 'try' when 'EUR' then 'eur' else 'dinar' end;
  v_field := case new.currency when 'USD' then 'openingUsd' when 'TRY' then 'openingTry' when 'EUR' then 'openingEur' else 'openingDinar' end;
  if not exists (
    ...
    where account.ledger_id = new.ledger_id and account.owner_id = new.owner_id
      ...
      and new.currency in ('LYD', 'USD', 'TRY', 'EUR')
...
create or replace function adreem_private.protect_account_opening_eur()
returns trigger
language plpgsql
security invoker
set search_path = ''
as $$
declare
  v_eur numeric := coalesce(nullif(new.payload ->> 'openingEur', '')::numeric, 0);
  v_old_eur numeric := case when tg_op = 'UPDATE' then coalesce(nullif(old.payload ->> 'openingEur', '')::numeric, 0) else 0 end;
  v_merge_target text := nullif(new.payload ->> 'mergedIntoAccountId', '');
begin
  if v_eur <> trunc(v_eur) or abs(v_eur) > 999999999999999 then
    raise exception 'ADREEM_INVALID_OPENING_AMOUNT' using errcode = '22023';
  end if;
  if new.currency_kind not in ('EUR', 'multi') and v_eur <> 0 then
    raise exception 'ADREEM_OPENING_CURRENCY_MISMATCH' using errcode = '23514';
  end if;
  if tg_op = 'UPDATE' and v_old_eur <> 0 and new.status = 'inactive' and v_merge_target is not null then
    if new.balance_eur <> 0 or not exists (
      select 1 from public.adreem_movements movement
      where movement.ledger_id = new.ledger_id and movement.owner_id = new.owner_id
        and movement.record_id = concat('opening-', new.record_id, '-eur')
        and movement.movement_type = 'opening_balance' and movement.currency = 'EUR'
        and movement.amount = v_old_eur and movement.destination_account_id = v_merge_target
    ...
    new.payload := jsonb_set(new.payload, '{openingEur}', '0'::jsonb, true);
    v_eur := 0;
  end if;
  if tg_op = 'UPDATE' and v_old_eur is distinct from v_eur then
    if not (v_old_eur <> 0 and v_eur = 0 and new.status = 'inactive' and v_merge_target is not null) then
      raise exception 'ADREEM_ACCOUNT_OPENING_IMMUTABLE' using errcode = '23514';
    end if;
  end if;
  return new;
...
revoke all on function adreem_private.protect_account_opening_eur() from public, anon, authenticated, service_role;
drop trigger if exists adreem_protect_account_opening_eur on public.adreem_accounts;
create trigger adreem_protect_account_opening_eur
before insert or update on public.adreem_accounts
for each row execute function adreem_private.protect_account_opening_eur();
...
declare
  ...
  v_amount numeric;
begin
  foreach v_currency in array array['LYD', 'USD', 'TRY', 'EUR'] loop
    v_field := case v_currency when 'USD' then 'openingUsd' when 'TRY' then 'openingTry' when 'EUR' then 'openingEur' else 'openingDinar' end;
  ...
  from (
    select
      ...
      (
        ...
        + abs(coalesce(sum(abs(movement.amount)) filter (where movement.currency = 'EUR' and movement.movement_type in ('external_income', 'truck_income')), 0)
          - coalesce(sum(abs(movement.amount)) filter (where movement.currency = 'EUR' and movement.movement_type in ('expense', 'truck_expense')), 0))
      ) as weight,
      jsonb_build_object(
        ...
        'netTry', coalesce(sum(abs(movement.amount)) filter (where movement.currency = 'TRY' and movement.movement_type in ('external_income', 'truck_income')), 0)
          - coalesce(sum(abs(movement.amount)) filter (where movement.currency = 'TRY' and movement.movement_type in ('expense', 'truck_expense')), 0),
        'incomeEur', coalesce(sum(abs(movement.amount)) filter (where movement.currency = 'EUR' and movement.movement_type in ('external_income', 'truck_income')), 0),
        'expenseEur', coalesce(sum(abs(movement.amount)) filter (where movement.currency = 'EUR' and movement.movement_type in ('expense', 'truck_expense')), 0),
        'netEur', coalesce(sum(abs(movement.amount)) filter (where movement.currency = 'EUR' and movement.movement_type in ('external_income', 'truck_income')), 0)
          - coalesce(sum(abs(movement.amount)) filter (where movement.currency = 'EUR' and movement.movement_type in ('expense', 'truck_expense')), 0)
      ) as payload
  ...
  from (
    select
      ...
      jsonb_build_object(
        ...
        'eur', coalesce(sum(abs(movement.amount)) filter (where movement.currency = 'EUR'), 0),
        'count', count(*)::bigint
      ) as payload
