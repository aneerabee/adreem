...
const LEGACY_STORAGE_KEY = String.fromCharCode(109, 111, 104, 97, 109, 109, 97, 100, 45, 108, 101, 100, 103, 101, 114, 45, 118, 49)
const LEGACY_BACKUP_STORAGE_KEY = String.fromCharCode(109, 111, 104, 97, 109, 109, 97, 100, 45, 108, 101, 100, 103, 101, 114, 45, 98, 97, 99, 107, 117, 112, 115, 45, 118, 49)
...
async function persistenceWithApi(initial = {}) {
  ...
  vi.resetModules()
  return { browser, module: await import('./ledgerPersistence.js') }
}
...
describe('ADREEM cloud-only persistence', () => {
  it('refuses to open a local ledger when the cloud API is not configured', async () => {
    ...
    vi.resetModules()
    const module = await import('./ledgerPersistence.js')

    expect(module.getLedgerPersistenceMode()).toBe('configuration-error')
    const loaded = await module.loadPersistedLedgerState({ accounts: [], movements: [] })
  ...
  it('removes legacy ledger caches but keeps the remembered login', async () => {
    const { store } = installBrowser({
      ...
      [`${LEGACY_BACKUP_STORAGE_KEY}:old`]: 'legacy-backup',
    ...
    expect(store.has(`${LEGACY_BACKUP_STORAGE_KEY}:old`)).toBe(false)
  ...
  it('saves only through the authenticated API', async () => {
    ...
    const result = await module.savePersistedLedgerState({ accounts: [{ id: 'cloud-only' }], movements: [] })
  ...
  it('deletes an unused account at the loaded revision and remembers the authoritative response', async () => {
    ...
    await module.loadPersistedLedgerState({ accounts: [], movements: [] })
  ...
  it('does not fall back to browser data when the login token is missing', async () => {
    ...
    expect(module.getLedgerPersistenceMode()).toBe('api-missing-token')
    const loaded = await module.loadPersistedLedgerState({ accounts: [], movements: [] })
    const saved = await module.savePersistedLedgerState({ accounts: [{ id: 'draft' }], movements: [] })
  ...
  it('keeps the remembered login active across browser sessions', async () => {
    ...
    expect(module.getLedgerPersistenceMode()).toBe('api')
  ...
  it('sends the cloud version received at load and advances it after save', async () => {
    ...
    await module.loadPersistedLedgerState({ accounts: [], movements: [] })
    await module.savePersistedLedgerState({ accounts: [], movements: [] })
    await module.savePersistedLedgerState({ accounts: [], movements: [] })
  ...
  it('keeps concurrent new movements when recovering from a cloud conflict', async () => {
    ...
    await module.loadPersistedLedgerState(baseState)
    const result = await module.savePersistedLedgerState({
      ...baseState,
  ...
  it('does not duplicate a movement when the server saved it but the response was lost', async () => {
    ...
    await module.loadPersistedLedgerState(baseState)
    const lostResponse = await module.savePersistedLedgerState(nextState)
    const recovered = await module.savePersistedLedgerState(nextState)
  ...
  it('marks state conflicts as permanent until the user reloads or retries deliberately', async () => {
    ...
    const result = await module.savePersistedLedgerState({ accounts: [], movements: [] })
  ...
  it('retries a relational delta when another client changed a different record', async () => {
    const { module } = await persistenceWithApi({ 'adreem-ledger-api-login-token-v1': 'valid-token' })
    const baseState = {
      accounts: [
        { id: 'web-account', name: 'before', updatedAt: '2026-08-20T18:00:00.000Z' },
        { id: 'bot-account', name: 'before', updatedAt: '2026-08-20T18:00:00.000Z' },
      ],
      movements: [],
    }
    const webAccount = { ...baseState.accounts[0], name: 'web edit', updatedAt: '2026-08-20T18:01:00.000Z' }
    const botAccount = { ...baseState.accounts[1], name: 'bot edit', updatedAt: '2026-08-20T18:02:00.000Z' }
    const remoteState = { ...baseState, accounts: [baseState.accounts[0], botAccount] }
    const fetchMock = vi.fn()
      .mockResolvedValueOnce({
        ok: true,
        json: async () => ({ state: baseState, storageMode: 'relational', revision: 7 }),
      })
      .mockResolvedValueOnce({
        ok: false,
        status: 409,
      ...
      .mockResolvedValueOnce({
        ok: true,
        json: async () => ({ state: remoteState, storageMode: 'relational', revision: 8 }),
      })
      .mockImplementationOnce(async (_url, options = {}) => {
        const request = JSON.parse(options.body)
        return {
          ok: true,
          json: async () => ({
            state: { ...remoteState, accounts: [request.delta.accounts[0], botAccount] },
            storageMode: 'relational',
            revision: 9,
          }),
        }
    ...
    const result = await module.savePersistedLedgerState({
      ...baseState,
      accounts: [webAccount, baseState.accounts[1]],
    })
    ...
    expect(JSON.parse(putCalls[1][1].body)).toMatchObject({
      baseRevision: 8,
      delta: { accounts: [expect.objectContaining({ id: 'web-account', name: 'web edit' })] },
    })
    expect(result.supabaseOk).toBe(true)
    expect(result.revision).toBe(9)
    expect(result.state.accounts).toEqual(expect.arrayContaining([
      expect.objectContaining(webAccount),
      expect.objectContaining(botAccount),
    ]))
  })

  it('does not treat database-derived balance changes as a profile edit conflict', async () => {
    const { module } = await persistenceWithApi({ 'adreem-ledger-api-login-token-v1': 'valid-token' })
    const baseAccount = {
      id: 'cash',
      ownerName: 'قبل',
    ...
    const fetchMock = vi.fn()
      ...
      .mockResolvedValueOnce({
        ok: true,
        json: async () => ({ state: { ...baseState, accounts: [remoteAccount] }, storageMode: 'relational', revision: 8 }),
      })
      .mockImplementationOnce(async (_url, options = {}) => {
        const request = JSON.parse(options.body)
        return {
    ...
    await module.loadPersistedLedgerState(baseState)
    const result = await module.savePersistedLedgerState({ ...baseState, accounts: [webAccount] })
  ...
  it('loads an exact older movement before merging a relational conflict', async () => {
    const { module } = await persistenceWithApi({ 'adreem-ledger-api-login-token-v1': 'valid-token' })
    const movements = Array.from({ length: 101 }, (_, index) => ({
      id: index === 0 ? 'older-target' : `movement-${index}`,
      note: 'base',
      databaseSequence: index + 1,
      updatedAt: '2026-08-20T10:00:00.000Z',
    }))
    const baseState = { accounts: [], movements }
    const editedTarget = { ...movements[0], note: 'edited', updatedAt: '2026-08-20T11:00:00.000Z' }
    const fetchMock = vi.fn()
      .mockResolvedValueOnce({
        ok: true,
        json: async () => ({ state: baseState, storageMode: 'relational', revision: 7 }),
      ...
      .mockResolvedValueOnce({
        ok: true,
        json: async () => ({
          state: { ...baseState, movements: [movements[0]] },
          storageMode: 'relational',
          revision: 8,
        }),
      })
      .mockResolvedValueOnce({
        ok: true,
        json: async () => ({ state: { ...baseState, movements: [] }, storageMode: 'relational', revision: 9 }),
      })
    vi.stubGlobal('fetch', fetchMock)
    ...
    const result = await module.savePersistedLedgerState({
      ...baseState,
      movements: [editedTarget, ...movements.slice(1)],
    })

    expect(String(fetchMock.mock.calls[2][0])).toContain('/api/ledger?movementId=older-target')
    expect(result.supabaseOk).toBe(true)
    expect(result.state.movements).toHaveLength(101)
    expect(result.state.movements.find((movement) => movement.id === 'older-target')?.note).toBe('edited')
  })

  it('recovers a relational conflict in bounded batches when more than 250 movements changed', async () => {
    const { module } = await persistenceWithApi({ 'adreem-ledger-api-login-token-v1': 'valid-token' })
    const movements = Array.from({ length: 251 }, (_, index) => ({
      id: `bulk-${index}`,
      note: 'base',
      databaseSequence: index + 1,
      updatedAt: '2026-08-20T10:00:00.000Z',
    ...
    const baseState = { accounts: [], movements }
    const editedMovements = movements.map((movement) => ({
      ...movement,
      note: 'edited',
      updatedAt: '2026-08-20T11:00:00.000Z',
    }))
    let initialLoadComplete = false
    let firstPutComplete = false
    const conflictBatchSizes = []
    const fetchMock = vi.fn(async (url, options = {}) => {
      if (!initialLoadComplete) {
        initialLoadComplete = true
        return { ok: true, json: async () => ({ state: baseState, storageMode: 'relational', revision: 7 }) }
      }
      if (options.method === 'PUT' && !firstPutComplete) {
        firstPutComplete = true
        return {
          ok: false,
          status: 409,
          headers: { get: () => null },
      ...
      if (options.method !== 'PUT') {
        const ids = new URL(String(url), 'https://app.test').searchParams.getAll('movementId')
        conflictBatchSizes.push(ids.length)
        return {
          ok: true,
          json: async () => ({
            state: { ...baseState, movements: movements.filter((movement) => ids.includes(movement.id)) },
            storageMode: 'relational',
            revision: 8,
          }),
        }
      ...
      return {
        ok: true,
        json: async () => ({
          state: { ...baseState, movements: editedMovements },
          storageMode: 'relational',
          revision: 9,
        }),
      }
    ...
    await module.loadPersistedLedgerState(baseState)
    const result = await module.savePersistedLedgerState({ ...baseState, movements: editedMovements })

    expect(result.supabaseOk).toBe(true)
    expect(result.revision).toBe(9)
    expect(conflictBatchSizes).toHaveLength(7)
    expect(Math.max(...conflictBatchSizes)).toBeLessThanOrEqual(40)
    expect(conflictBatchSizes.reduce((sum, count) => sum + count, 0)).toBe(251)
  })
  ...
  it('keeps accumulated page attachments when a save response contains only the first page', async () => {
    ...
    await module.loadPersistedLedgerState({ accounts: [], movements: [], attachments: [] })
    const page = await module.loadAdreemMovementPage({ before: 2 })
    const saved = await module.savePersistedLedgerState({
      accounts: [{ id: 'cash', updatedAt: '2026-08-20T19:00:00.000Z' }],
  ...
  it('ignores a movement page that arrives after a newer relational save', async () => {
    ...
    await vi.waitFor(() => expect(fetchMock).toHaveBeenCalledTimes(2))
    const save = await module.savePersistedLedgerState({ accounts: [], movements: [initialMovement, savedMovement] })
    resolvePage({
      ok: true,
      json: async () => ({
  ...
  it('loads a filtered history page without resubmitting fetched records as new movements', async () => {
    ...
    const save = await module.savePersistedLedgerState({ accounts: [], movements: page.allMovements })
