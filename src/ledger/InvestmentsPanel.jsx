...
export default function InvestmentsPanel({
  ...
  const [tradeLaunch, setTradeLaunch] = useState(null)
  const [holdingIntent, setHoldingIntent] = useState('')
    ...
    : holdingDraft.assetType === INVESTMENT_ASSET_TYPES.METAL
      ...
      : holdingDraft.quoteCurrency === 'TRY'
        ...
        : holdingDraft.quoteCurrency === 'USD' && import.meta.env.VITE_ADREEM_STOCK_DISPLAY_LICENSED !== 'true' && ['stock', 'fund'].includes(holdingDraft.assetType)
          ? 'رمز السهم مثل AAPL'
        : 'الاسم أو الرمز'
  const emptyMarketSearchMessage = holdingDraft.assetType === INVESTMENT_ASSET_TYPES.CRYPTO && /[\u0600-\u06ff]/u.test(assetQuery)
    ? 'اكتب اسم العملة بالإنجليزية، مثل Polkadot.'
    : 'لا توجد نتيجة. اكتب الاسم والرمز يدويًا.'
  ...
  const tryRateKey = dialog === 'holding' && holdingIntent !== 'buy' && holdingDraft.quoteCurrency === 'TRY'
    ? 'holding'
    : dialog === 'price' && holdingById.get(manualHoldingId)?.quoteCurrency === 'TRY' ? `price:${manualHoldingId}` : ''
  const { tryFx, changeTryFxRate, tryFxIsReadyForSave, resetTryFxReload } = useTryUsdRate(tryRateKey, onLoadTryUsdRate)
  ...
  function openHolding(platformId = activePlatforms[0]?.id || '', intent = '') {
    submissionRef.current = false
    resetTryFxReload()
    setHoldingIntent(intent)
    setHoldingDraft({ ...blankHolding, platformId })
    setAssetQuery('')
    setAssetResults([])
  ...
  function openTrade(platformId, type = INVESTMENT_TRADE_TYPES.BUY, holdingId = '') {
    setTradeLaunch((current) => ({ platformId, type, holdingId, key: (current?.key || 0) + 1 }))
    setDialog('trade')
  }
  ...
  function submitHolding(event) {
    ...
    if (holdingIntent !== 'buy' && holdingDraft.quoteCurrency === 'TRY' && holdingHasOpening && !tryFxIsReadyForSave()) return
    const purchaseHoldingId = holdingIntent === 'buy' ? newInvestmentHoldingId() : ''
    const baseDraft = purchaseHoldingId
      ? { ...holdingDraft, id: purchaseHoldingId, initialQuantity: '', initialPriceUsd: '', initialPriceNative: '' }
      : holdingDraft
    const draft = baseDraft.marketDataMode === 'manual'
      ? { ...baseDraft, providerSymbol: baseDraft.symbol.trim().toUpperCase() }
      : baseDraft
    const pricedDraft = !purchaseHoldingId && holdingDraft.quoteCurrency === 'TRY' && holdingHasOpening
    ...
    if (purchaseHoldingId) {
      openTrade(draft.platformId, INVESTMENT_TRADE_TYPES.BUY, purchaseHoldingId)
      return
    }
    setDialog('')
  }
  ...
  function openManualPrice(holding) {
    ...
    const priceMicros = holding.quoteCurrency === 'TRY' ? holding.lastPriceNativeMicros : holding.lastPriceUsdMicros
    setManualPrice(priceMicros ? String(microsToUsd(priceMicros)) : '')
    setDialog('price')
  }

  function submitManualPrice(event) {
    event.preventDefault()
    if (!manualHoldingId || !manualPriceIsValid || submissionRef.current) return
    if (manualIsTurkish && !tryFxIsReadyForSave()) return
    submissionRef.current = true
    const priceInput = manualIsTurkish ? { priceNative: manualPrice, tryPerUsd: tryFx.rate } : { priceUsd: manualPrice }
    if (onManualPrice(manualHoldingId, priceInput) === false) {
      submissionRef.current = false
      return
    }
    setDialog('')
  ...
  function submitTradeEdit(event) {
    ...
    submissionRef.current = true
    if (onEditTrade?.(editingTradeBaseline, { ...tradeEditDraft, feeUsd: String(microsToUsd(editedFeeUsdMicros)) }) === false) {
      submissionRef.current = false
      return
    }
    submissionRef.current = false
  ...
  const manualIsTurkish = manualHolding?.quoteCurrency === 'TRY'
  const manualPriceUsdPreview = manualIsTurkish ? convertTryPriceToUsdMicros(usdToMicros(manualPrice), tryRateMicros) : usdToMicros(manualPrice)
  const manualPriceIsValid = parseInvestmentDecimal(manualPrice) > 0
    && (!manualIsTurkish || (tryRateMicros > 0 && tryFx.quotedAt && !['loading', 'error'].includes(tryFx.status)))
  ...
  const tradeLaunchRow = tradeLaunch ? summary.platforms.find((row) => row.platform.id === tradeLaunch.platformId) || null : null
                      ...
                      >
                        ...
                        {detailsOpen ? <div className="adreem-investment-holding-details">
                          ...
                          <div className="adreem-investment-detail-measures">
                            {row.native?.averageCostMicros ? <span><small>متوسط الشراء TRY</small><strong>{tryUnitMicros(row.native.averageCostMicros)}</strong></span> : null}
                            <span><small>{row.native ? 'متوسط الشراء USD' : 'متوسط الشراء'}</small><strong>{usdUnitMicros(row.averageCostUsdMicros)}</strong></span>
                          ...
                          {row.native && row.native.costBasisMicros === null && row.quantityUnits > 0 ? <p className="adreem-investment-native-note">نتيجة الليرة غير متاحة: شراء سابق سُجّل بالدولار دون سعر الليرة.</p> : null}
      ...
      <AnimatePresence mode="wait">
          ...
          <InvestmentDialog key="holding" title={holdingIntent === 'buy' ? 'أصل جديد للشراء' : 'استثمار جديد'} subtitle={holdingIntent === 'buy' ? preserveUiData(platformById.get(holdingDraft.platformId)?.name || '') : 'ابحث أو اكتب الأصل يدويًا'} icon={ChartCandlestick} tone="market" className="is-holding" onClose={() => setDialog('')} onSecondary={holdingIntent === 'buy' ? () => openTrade(holdingDraft.platformId) : undefined} submitLabel={holdingIntent === 'buy' ? 'متابعة للشراء' : undefined} onSubmit={submitHolding} canSubmit={Boolean(holdingDraft.platformId && holdingDraft.name.trim() && /^[A-Z0-9./_-]{1,32}$/.test(holdingDraft.symbol.trim().toUpperCase()) && (holdingDraft.marketDataMode === 'manual' || holdingDraft.providerSymbol.trim()) && holdingOpeningValid)}>
            ...
            <div className="is-paired">
              ...
              <div className="adreem-investment-market-choice">
                <span>{assetUsesCurrencyMarket(holdingDraft.assetType) ? 'عملة السعر' : 'سوق التداول'}</span>
                <div role="radiogroup" aria-label={assetUsesCurrencyMarket(holdingDraft.assetType) ? 'عملة السعر' : 'سوق التداول'}>
                  {MARKET_OPTIONS.map((market) => <button type="button" role="radio" aria-checked={holdingDraft.quoteCurrency === market.value} className={holdingDraft.quoteCurrency === market.value ? 'is-active' : ''} key={market.value} onClick={() => changeHoldingMarket(market.value)}><strong>{assetUsesCurrencyMarket(holdingDraft.assetType) ? market.currencyLabel : market.label}</strong><small>{market.value}</small></button>)}
                </div>
              </div>
            </div>
        ...
        {dialog === 'trade' && tradeLaunchRow ? (
          <InvestmentTradeDialog
            ...
            initialType={tradeLaunch.type}
            initialHoldingId={tradeLaunch.holdingId}
            onAddTrade={onAddTrade}
            onOpenFunding={onOpenFunding}
            onAddAsset={(platformId) => openHolding(platformId, 'buy')}
            onLoadTryUsdRate={onLoadTryUsdRate}
            onClose={() => setDialog('')}
          />
        ) : null}
        {dialog === 'price' ? (
