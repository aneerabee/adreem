...
export default function AdreemChrome({
  ...
  const saveShieldRef = useRef(null)
  ...
  useEffect(() => {
    if (!saveBlocked) return undefined
    const focusBeforeBlock = document.activeElement
    const focusTimer = window.requestAnimationFrame(() => {
      const firstControl = saveShieldRef.current?.querySelector('button:not(:disabled)')
      ;(firstControl || saveShieldRef.current)?.focus({ preventScroll: true })
    })
    function keepSaveFocus(event) {
      if (event.key !== 'Tab') return
      const controls = Array.from(saveShieldRef.current?.querySelectorAll('button:not(:disabled), [tabindex]:not([tabindex="-1"])') || [])
      if (!controls.length) {
        event.preventDefault()
        saveShieldRef.current?.focus({ preventScroll: true })
        return
      }
      const first = controls[0]
      const last = controls[controls.length - 1]
    ...
    document.addEventListener('keydown', keepSaveFocus)
    return () => {
      window.cancelAnimationFrame(focusTimer)
      document.removeEventListener('keydown', keepSaveFocus)
      focusBeforeBlock?.focus?.({ preventScroll: true })
    }
  }, [saveBlocked, saveStatus])

  return (
    <main className={`adreem-app adreem-app--${activeSection}`} dir={direction} lang={normalizedLanguage}>
      <section className="adreem-shell">
        <header className="adreem-header" inert={saveBlocked}>
        ...
        <div className="adreem-frame">
          <nav className="adreem-nav" aria-label="أقسام الدفتر" inert={saveBlocked}>
          ...
          <section className="adreem-view" inert={saveBlocked}>{children}</section>
          {saveBlocked ? (
            <div ref={saveShieldRef} tabIndex={-1} className={`adreem-save-shield adreem-save-shield--${saveStatus}`} role={saveFailed ? 'alertdialog' : 'status'} aria-live="assertive" aria-modal={saveFailed ? 'true' : undefined}>
              <section>
                ...
                {saveStatus !== 'saving' ? (
                  <div className="adreem-save-shield-actions">
                    ...
                    {saveFailed && canLogout ? (
                      <button type="button" className="is-secondary" onClick={onLogout}>
