...
describe('ADREEM UI translation', () => {
  ...
  it('preserves user-entered names and notes', () => {
    expect(translateUiText('أحمد الكيفو', 'en')).toBe('أحمد الكيفو')
  ...
  it('translates system labels around Arabic user data without changing that data', () => {
    expect(translateUiText('الحساب الذي خرجت منه الفلوس: أحمد · كاش بيننا', 'en'))
      .toBe('Money came from: أحمد · Cash between us')
    expect(translateUiText('حذف دخول أحمد؟ بيانات الدفتر لن تُحذف.', 'en'))
      .toBe('Remove login for أحمد? Ledger data will not be deleted.')
  })
})
