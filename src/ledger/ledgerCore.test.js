...
import { ACCOUNT_STATUSES, ACCOUNT_TYPES, VALUE_KINDS, adreemAccountCatalog, adreemSummaryAccounts } from './accountCatalog'
import {
  CURRENCIES,
  MOVEMENT_STATUSES,
  MOVEMENT_TYPES,
...
describe('adreem ledger core', () => {
  ...
  it('creates opening balances from the Numbers catalog without losing cash or bank separation', () => {
    const openings = createOpeningMovements(adreemAccountCatalog)
    const balances = summarizeBalances(adreemAccountCatalog, openings)

    expect(getAccountBalance('me-cash', adreemAccountCatalog, openings).dinar).toBe(50000)
    expect(getAccountBalance('me-cash', adreemAccountCatalog, openings).usd).toBe(500)
    expect(getAccountBalance('me-jumhouria', adreemAccountCatalog, openings).dinar).toBe(30000)
  ...
  it('adds a newly posted local movement over database balances before cloud confirmation', () => {
    const accounts = [{
      id: 'cash',
      status: ACCOUNT_STATUSES.ACTIVE,
      balanceSource: 'database',
    ...
    expect(summarizeBalances(accounts, [localMovement])[0]).toMatchObject({
      dinar: 1400,
      postedCount: 10,
    })
  })

  it('preserves database balances when a new unsaved account is added', () => {
    const storedAccount = {
      id: 'cash',
      status: ACCOUNT_STATUSES.ACTIVE,
      balanceSource: 'database',
    ...
    const newAccount = { id: 'new-person', status: ACCOUNT_STATUSES.ACTIVE }

    const balances = summarizeBalances([storedAccount, newAccount], [])

    expect(balances.find((bucket) => bucket.account.id === 'cash')).toMatchObject({ dinar: 1200, postedCount: 9 })
    expect(balances.find((bucket) => bucket.account.id === 'new-person')).toMatchObject({ dinar: 0, postedCount: 0 })
  })

  it('shows the optimistic balance difference for a saved movement edit or cancellation', () => {
    const accounts = [{
      id: 'cash',
      status: ACCOUNT_STATUSES.ACTIVE,
      balanceSource: 'database',
    ...
    const edited = markOptimisticMovementChange({ ...original, amount: 300 }, original)
    const cancelled = markOptimisticMovementChange({ ...original, status: MOVEMENT_STATUSES.VOIDED }, original)

    expect(summarizeBalances(accounts, [edited])[0]).toMatchObject({ dinar: 1300, postedCount: 9 })
    expect(summarizeBalances(accounts, [cancelled])[0]).toMatchObject({ dinar: 1000, postedCount: 8 })
  })

  it('keeps the persisted movement as the balance base across repeated local edits', () => {
    const accounts = [{
      id: 'cash',
      balanceSource: 'database',
      balanceDinar: 1200,
    ...
    const firstEdit = markOptimisticMovementChange({ ...persisted, amount: 300 }, persisted)
    const secondEdit = markOptimisticMovementChange({ ...firstEdit, amount: 400 }, firstEdit)

    expect(summarizeBalances(accounts, [secondEdit])[0]).toMatchObject({ dinar: 1400, postedCount: 1 })
  })
  ...
  it('keeps incomplete movements out of posted balances', () => {
    ...
    const balance = getAccountBalance('saeed-cash', adreemAccountCatalog, [...openings, badMovement])
  ...
  it('rejects summary accounts as posting endpoints', () => {
    const movement = postMovement(
      ...
      [...adreemAccountCatalog, ...adreemSummaryAccounts],
    )
  ...
  it('supports voiding a posted movement without deleting it', () => {
    ...
    const withMovement = getAccountBalance('omar-gold', adreemAccountCatalog, [...openings, movement])
    expect(withMovement.dinar).toBe(21000)

    const result = voidMovement(movement, 'إدخال بالخطأ')
    expect(result.ok).toBe(true)
    const afterVoid = getAccountBalance('omar-gold', adreemAccountCatalog, [...openings, result.movement])
  ...
  it('does not allow usd sale or purchase without a valid exchange rate', () => {
    const preview = previewMovement(
      ...
      adreemAccountCatalog,
      createOpeningMovements(adreemAccountCatalog),
    )
  ...
  it('rejects exchange movements whose converted side rounds to zero', () => {
    const accounts = [
      createAccount({ id: 'usd', ownerName: 'أنا', subAccountName: 'دولار', type: ACCOUNT_TYPES.CASH, valueKind: VALUE_KINDS.CASH, currencyKind: CURRENCIES.USD, openingUsd: 10 }),
      createAccount({ id: 'dinar', ownerName: 'أنا', subAccountName: 'دينار', type: ACCOUNT_TYPES.CASH, valueKind: VALUE_KINDS.CASH, currencyKind: CURRENCIES.DINAR, openingDinar: 10 }),
    ]

    const sale = validateMovement({
      type: MOVEMENT_TYPES.USD_SALE,
      amount: 1,
      currency: CURRENCIES.USD,
      rate: 0.1,
      sourceAccountId: 'usd',
      destinationAccountId: 'dinar',
    }, accounts)
    const purchase = validateMovement({
      type: MOVEMENT_TYPES.USD_PURCHASE,
      amount: 1,
      currency: CURRENCIES.DINAR,
      rate: 5,
      sourceAccountId: 'dinar',
      destinationAccountId: 'usd',
    }, accounts)

    expect(sale.errors).toContainEqual(expect.objectContaining({ field: 'rate' }))
    expect(purchase.errors).toContainEqual(expect.objectContaining({ field: 'rate' }))
  })
  ...
  it('labels balance direction based on account kind', () => {
    const person = adreemAccountCatalog.find((account) => account.id === 'rabee-cash')
    const bank = adreemAccountCatalog.find((account) => account.id === 'me-jumhouria')
    const expense = adreemAccountCatalog.find((account) => account.id === 'personal-expense')
    const asset = adreemAccountCatalog.find((account) => account.type === ACCOUNT_TYPES.ASSET)
  ...
  it('creates dynamic accounts with validation before use', () => {
    ...
    expect(validateAccount(account, adreemAccountCatalog).ok).toBe(true)
    expect(validateAccount({ ...account, ownerName: '' }, adreemAccountCatalog).ok).toBe(false)
  })
  ...
  it('rejects duplicate active accounts with the same owner and detail', () => {
    ...
    expect(validateAccount(account, adreemAccountCatalog).ok).toBe(false)
