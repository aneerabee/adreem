...
describe('investments panel', () => {
  ...
  it('keeps platform, holding, value, and profit visually distinct', () => {
    ...
    expect(html).not.toContain('>تسجيل</span>')
    expect(html.match(/aria-label="شراء أو بيع"/g)).toHaveLength(1)
    expect(html.indexOf('aria-label="شراء أو بيع"')).toBeLessThan(html.indexOf('adreem-investment-holdings'))
    expect(html).not.toContain('class="is-quantity"')
    expect(html).not.toContain('متوسط الشراء')
    expect(html).toContain('aria-expanded="false"')
    expect(html).not.toContain('تسجيل شراء أو بيع')
