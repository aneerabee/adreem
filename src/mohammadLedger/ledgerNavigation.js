...
const PEOPLE_VIEW_KEYS = new Set(['balances', 'all'])
...
export function readLedgerNavigation(search = '') {
  ...
  return {
    ...
    peopleView: allowed(params.get('people'), PEOPLE_VIEW_KEYS, DEFAULT_LEDGER_NAVIGATION.peopleView),
  }
}
...
export function ledgerNavigationSearch(search = '', navigation = DEFAULT_LEDGER_NAVIGATION) {
  ...
  if (next.section === 'accounts' && next.accountGroup === 'people' && next.peopleView !== DEFAULT_LEDGER_NAVIGATION.peopleView) {
    params.set('people', next.peopleView)
  } else {
