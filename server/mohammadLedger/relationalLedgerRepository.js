...
import { createLedgerDelta, isLedgerDeltaEmpty } from '../../src/mohammadLedger/ledgerDelta.js'
import { createLedgerIdentity, normalizeLedgerState } from '../../src/mohammadLedger/ledgerState.js'
...
export function createRelationalLedgerRepository(client, options = {}) {
  ...
  async function loadSnapshot(loadOptions = {}) {
    ...
    const [movementResult, reviewResult, accounts, dimensions, attachments, recurringRules, reconciliations, auditEvents, ignoredRows] = await Promise.all([
      movementRequest,
      reviewRequest,
      fetchAll(client, 'adreem_accounts', 'record_id, payload, balance_dinar, balance_usd, posted_count, structure_locked', ledger.id),
