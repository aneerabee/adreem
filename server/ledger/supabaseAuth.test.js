...
describe('Supabase Auth sessions', () => {
  ...
  it('revokes complete and refresh-only sessions and handles failure paths safely', async () => {
    ...
    await expect(success.service.logout('access', 'refresh')).resolves.toBe(true)
    ...
    await expect(success.service.logout('', 'refresh')).resolves.toBe(true)
    ...
    await expect(restoreFailed.service.logout('access', 'refresh')).resolves.toBe(true)
    ...
    await expect(refreshFailed.service.logout('', 'refresh')).resolves.toBe(false)
...
describe('Supabase Auth user administration', () => {
  ...
  it('creates a confirmed user with normalized identity metadata and returns its isolated ledger', async () => {
    ...
    expect(spies.createUser).toHaveBeenCalledWith({
      ...
      user_metadata: { display_name: 'أحمد', language: 'en' },
      app_metadata: {
        adreem_member: true,
