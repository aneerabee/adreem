...
describe('ADREEM relational migration projection', () => {
  ...
  it('validates all movement paths and creates bounded ordered batches', () => {
    ...
    expect(validation.ok).toBe(true)
    expect(validation.totals.get('cash')).toEqual({ dinar: 8_950, usd: 0, try: 0, eur: 0, postedCount: 10 })
    expect(validation.totals.get('usd')).toEqual({ dinar: 0, usd: 1_000, try: 0, eur: 0, postedCount: 3 })
