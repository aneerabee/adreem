...
import { VALUE_KINDS, getActivePostingAccounts } from '../../src/ledger/accountCatalog.js'
import { accountDisplayName } from '../../src/ledger/accountConfig.js'
import {
  CURRENCIES,
  MOVEMENT_STATUSES,
  buildPostingEntries,
...
} from '../../src/ledger/movementAccounts.js'
...
function appendTelegramRecurringRule(recurringRules = [], movement, draft = {}) {
  ...
  const rule = createRecurringRuleFromMovement(movement, {
    name: draft.recurringName || draft.note || '',
    firstRunOn: draft.recurringFirstRunOn || '',
  })
  if (!rule) return recurringRules
  const existingRules = Array.isArray(recurringRules) ? recurringRules : []
  const hasSameRule = existingRules.some((item) => item?.sourceMovementId === movement.id)
