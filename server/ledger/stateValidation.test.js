...
describe('server ledger state validation', () => {
  ...
  it('rejects silently deleting one linked channel or the complete person bundle', () => {
    ...
    expect(allRemoved.errors.filter((error) => error.code === 'account-deletion-not-allowed')).toHaveLength(5)
  })
  ...
  it('rejects editing the financial values of a posted movement older than 24 hours', () => {
    const oldExpense = {
      id: 'old-expense-edit',
      type: MOVEMENT_TYPES.EXPENSE,
      status: MOVEMENT_STATUSES.POSTED,
    ...
    const edited = { ...oldExpense, amount: 20, updatedAt: '2026-08-19T12:00:00.000Z' }

    const result = validateLedgerStateTransition({ ...current, movements: [opening(), edited] }, current, {
      now: '2026-08-19T12:00:00.000Z',
    })
  ...
  it.each([
    ...
    ['invalid-recurring-first-date', { firstRunOn: '2026-02-30' }],
    ['invalid-recurring-next-date', { nextRunOn: 'not-a-date' }],
    ['recurring-next-date-required', { firstRunOn: '2026-09-15' }],
    ['recurring-day-date-mismatch', { firstRunOn: '2026-09-15', nextRunOn: '2026-09-16' }],
    ['invalid-recurring-next-time', { nextRunAt: 'not-a-time' }],
