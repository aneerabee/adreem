...
# فعّل فقط بعد تأكيد حق العرض للمستخدمين في عقد مزود الأسعار
ADREEM_TWELVE_STOCK_DISPLAY_LICENSED=false
# نفس التأكيد في إعدادات بناء الويب، بلا مفتاح سري
VITE_ADREEM_STOCK_DISPLAY_LICENSED=false
...
مفتاح أسعار «محفظتي» يبقى على الخادم فقط. دون مفتاح وحق عرض مؤكد، تبقى الأسهم التركية والأوروبية بسعر يدوي؛ لا تستخدم نسخة المزود التجريبية لعرض الأسعار. للأسهم والصناديق الأمريكية المشمولة في TGMCharts يمكن اختيار الرمز الدقيق عند الإنشاء ليُعرض إغلاق يومي بالدولار مع التاريخ وذكر المصدر؛ [شروطه](https://tgmcharts.com/terms) تسمح بالعرض داخل التطبيقات مع ذكر المصدر ورابطه. الاستثمارات الأمريكية القديمة المسجلة يدويًا لا تتحول تلقائيًا، ولا يُفترض أن جميع الرموز مشمولة. تُعرض BTC وETH بسعر مرجعي بالدولار من Gold API، وكذلك المعادن XAU وXAG وXPT وXPD، بعد مطابقة الرمز والعملة والتوقيت. ليست هذه الأسعار سعر منصة تداول بعينها. تبقى العملات الرقمية الأخرى بسعر يدوي؛ ولا يُفترض تساوي USDT وUSD. إذا تعذر السعر أو توقيته، تبقى آخر قيمة محفوظة.

للأسهم والصناديق التركية المختارة من البحث الجديد، يستخدم الخادم Bürküt عندما يكون `ADREEM_BURKUT_API_KEY` موجودًا في الملف الخاص. تُجلب قائمة كل سوق مرة كل ساعتين تقريبًا لتوفير الحصة المجانية، ويُرفض السعر إذا اختلف الرمز أو النوع أو أشار المزود إلى بيانات قديمة. `updatedAt` هو وقت تحديث المزود وليس بالضرورة وقت تنفيذ صفقة؛ تعرض الواجهة هذا الفرق ولا تسميه سعر تداول حيًا. يبقى السعر السابق عند فشل المزود أو تحويل العملة. الاستثمارات القديمة والمدخلة يدويًا لا تنتقل إلى المصدر الجديد تلقائيًا. تحقق من حقوق عرض بيانات بورصة إسطنبول للمستخدمين قبل التوسع في استخدامها.

للعملات الرقمية غير المرجعية (مثل Polkadot وSolana)، يبحث الخادم ويجلب الأسعار من CoinGecko دون مفتاح. تظهر في البحث العملات المرتبة حسب القيمة السوقية فقط، ويُحفظ معرف العملة داخل رمز المصدر بصيغة `DOT/USD:CG-POLKADOT` حتى لا تختلط عملتان بالرمز نفسه. يُرفض السعر إذا كان أقدم من 15 دقيقة، أو إذا كان تداول آخر 24 ساعة أقل من 10,000 دولار، أو إذا كان أصغر من دقة الحفظ. البيتكوين والإيثيريوم وFET وAvalanche تبقى على مصادرها المرجعية الحالية. عند زيادة الاستخدام يمكن إضافة مفتاح CoinGecko التجريبي المجاني `COINGECKO_API_KEY` في الملف الخاص نفسه ثم إعادة تشغيل الخدمة.

لتدوير المفتاح لاحقًا دون إرساله لأحد: استبدل قيمة `ADREEM_BURKUT_API_KEY` داخل `/home/argaz/apps/adreem/adreem.env` على الخادم، واحتفظ بصلاحية الملف `0600`، ثم نفذ `systemctl --user restart adreem-api.service` واختبر تحديث سعر تركي. تغيير المفتاح في موقع المزود وحده لا يحدّث الخادم تلقائيًا. لا تضع المفتاح في Git أو إعدادات بناء الويب.

بعد تفعيل حق العرض ومفتاح المزود على الخادم، تُقرأ الأسهم التركية من إغلاق اليوم عبر `/eod`، والأمريكية من عرض السعر الموقّت عبر `/quote`. يستخدم الخادم السعر المرجعي اليومي من البنك المركزي الأوروبي لتحويل TRY أو EUR إلى USD: USD لكل EUR، وUSD لكل TRY مشتق من USD/EUR وTRY/EUR لنفس التاريخ فقط. يُعرض وقت الصرف منفصلًا ولا يوصف بأنه لحظي. يخزن الخادم النتيجة مؤقتًا ويرفض السعر القديم أو غير المكتمل.

تعتمد المحفظة على وقت السعر الذي يعيده المزود، لا على وقت وصول الرد. يُرفض السعر بلا توقيت موثوق أو إذا تأخر أثناء فتح السوق، وتبقى القيمة السابقة محفوظة. يُعرض وقت سعر الصرف منفصلًا عند تحويل سعر محلي إلى الدولار. يتحقق المتصفح من الأسعار عند استخدام المحفظة ثم كل ساعتين ما دام مفتوحًا؛ لا توجد مهمة خلفية على الخادم بعد إغلاق الصفحة.

قبل تفعيل أسعار الأسواق لمستخدمين آخرين: احصل على تأكيد مكتوب لحق العرض والتوزيع من مزود البيانات، وتأكد أن الخطة تغطي بورصة إسطنبول يوميًا والأسهم الأمريكية والصناديق المطلوبة. فعّل الإعدادين أعلاه معًا فقط بعد ذلك، وجرّب رموزًا حقيقية من كل فئة على الخادم. لا تخزن المفتاح في المتصفح أو المستودع ولا ترسله في محادثة. لا تخلط `USDT` بالدولار دون سعر صرف مؤكد.
