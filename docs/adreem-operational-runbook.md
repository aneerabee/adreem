...
مفتاح أسعار «محفظتي» يبقى على الخادم فقط. إذا لم تضبط `TWELVE_DATA_API_KEY` يستخدم الخادم وصول المزود التجريبي للأدوات المدعومة، وسعر تداول مباشر من Coinbase لأزواج الدولار المختارة منها، وسعر آخر تداول من Binance للأزواج المسعرة بـ USDT بعد تحويلها بسعر USDT/USD مؤكد من Coinbase. تستخدم المعادن XAU وXAG وXPT وXPD بالدولار سعرًا من Gold API بعد مطابقة الرمز والعملة والتوقيت، وإلا يُجرب المزود السابق. لا يُفترض تساوي USDT وUSD. إذا تعذر مصدر السعر أو توقيته أو تحويله تبقى آخر قيمة مؤكدة محفوظة.

إذا غاب سعر صرف TRY/USD أو EUR/USD من المزود الرئيسي، يستخدم الخادم السعر المرجعي اليومي من البنك المركزي الأوروبي: USD لكل EUR، وUSD لكل TRY مشتق من USD/EUR وTRY/EUR لنفس التاريخ فقط. يُعرض هذا المصدر في تفاصيل الاستثمار ولا يوصف بأنه سعر صرف لحظي. يخزن الخادم النتيجة مؤقتًا لتجنب الإفراط في الطلبات، ويرفض السعر المرجعي القديم أو غير المكتمل. لا تُستخدم أسعار العملات الرقمية من سوق آخر إلا للزوج المطابق لمصدر الاستثمار المحدد.
...
قبل تفعيل أسعار الأسواق لمستخدمين آخرين: تحقق من حق العرض والتوزيع في عقد مزود البيانات. تعرض Twelve Data أسعار بورصة إسطنبول بنهاية اليوم فقط، ولا تكفي لتحديث لحظي أثناء التداول. تحتاج الأسعار التركية المتغيرة إلى مزود مرخص يدعم بورصة إسطنبول، مع تأكيد نوع التأخير، ووقت السعر، وحدود الاستعمال. قد تتطلب الأسهم الأمريكية والمعادن وسعر صرف الليرة/الدولار والصناديق تغطيات مختلفة؛ جرّب رموزًا حقيقية من كل فئة على الخادم قبل وصفها بأنها تعمل. لا تخزن المفتاح في المتصفح أو المستودع، ولا ترسل المفتاح في محادثة. لا تخلط سعر زوج `USDT` بسعر الدولار دون سعر صرف مؤكد له.
